...
exports.uploadLogo = async (req, res) => {
  try {
    if (!req.file) {
      return res.status(400).json({ success: false, message: 'Logo fayli topilmadi.' });
    }
    ...
    // Eski logoni Cloudinarydan o'chirish (agar mavjud bo'lsa va yangi public_id boshqacha bo'lsa)
    const oldPublicId = settings.branding?.logoPublicId;
    if (oldPublicId && oldPublicId !== req.file.filename) {
      try {
        await cloudinary.uploader.destroy(oldPublicId);
      } catch (e) {
        console.warn('Eski logoni o\'chirishda xatolik (muhim emas):', e.message);
      }
    }

    // Yangi logo ma'lumotlarini saqlash
    const currentBranding = settings.branding?.toObject?.() || settings.branding || {};
    settings.branding = {
      ...currentBranding,
    ...
    await logAction(req, 'UPDATE', 'Settings', settings._id, `Branding logo yangilandi: ${req.file.path}`);
    ...
    console.error('uploadLogo xatosi:', error);
    res.status(500).json({ success: false, message: error.message });
  }
};
...
exports.deleteLogo = async (req, res) => {
  try {
    ...
    const publicId = settings.branding?.logoPublicId;
    if (publicId) {
      try {
        await cloudinary.uploader.destroy(publicId);
      } catch (e) {
        console.warn('Cloudinarydan o\'chirishda xatolik:', e.message);
      }
    }

    const currentBranding = settings.branding?.toObject?.() || settings.branding || {};
    settings.branding = { ...currentBranding, logoUrl: '', logoPublicId: '' };
    await settings.save();

    await logAction(req, 'UPDATE', 'Settings', settings._id, 'Branding logo o\'chirildi');

    res.status(200).json({ success: true, message: 'Logo muvaffaqiyatli o\'chirildi!' });
  } catch (error) {
    res.status(500).json({ success: false, message: error.message });
  }
};
...
exports.updateSettings = async (req, res) => {
  try {
    const { usdExchangeRate, cartFields, features, branding, productCategories, productUnits } = req.body;
    let settings = await Settings.findOne();
    ...
    if (!settings) {
      settings = await Settings.create({ 
        ...
        productUnits: productUnits || undefined
      });
    } else {
      ...
      // branding — white-label ma'lumotlarini yangilash (merge)
      // Eslatma: logoUrl va logoPublicId alohida /upload-logo endpoint'i orqali yangilanadi
      if (branding !== undefined) {
        const currentBranding = settings.branding?.toObject?.() || settings.branding || {};
        const { logoUrl, logoPublicId, ...brandingFields } = branding; // Logo fieldlarini ignore qilish
        settings.branding = { ...currentBranding, ...brandingFields };
      }
      ...
      if (productUnits !== undefined) {
        settings.productUnits = productUnits;
      }
    ...
    if (branding?.storeName) logDetails.push(`Do'kon nomi: ${branding.storeName}`);
