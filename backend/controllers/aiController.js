...
exports.getAiAnalytics = async (req, res) => {
  try {
    ...
    orders.forEach(order => {
      overallRevenue += (order.totalAmount || 0);
      overallProfit += (order.totalProfit || 0);
      overallDebt += (order.debtAmount || 0);
    ...
    const prompt = `Siz O'zbekiston bozori uchun ixtisoslashgan katta biznes-analitik va maslahatchi sifatida faoliyat yuritasiz. Quyidagi savdo va ombor ma'lumotlarini chuqur tahlil qiling va rahbariyat uchun professional hisobot taqdim eting.
...
[RECOMMENDATIONS]
Strategik tavsiyalar: Muammolarni hal qilish va moliyaviy barqarorlikni ta'minlash uchun aniq, amaliy 3-5 ta tavsiya. Har doim ro'yxat (bullet points) shaklida yozing. Har bir tavsiya mantiqan asoslangan va londa bo'lsin.
...
=== MOLIYAVIY HOLAT (So'nggi 90 kun) ===
- Jami tushum: ${overallRevenue.toLocaleString()} so'm
- Sof foyda: ${overallProfit.toLocaleString()} so'm
- Nasiya / Qarzlar: ${overallDebt.toLocaleString()} so'm
