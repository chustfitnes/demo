...
exports.exportFullDatabaseExcel = async (req, res) => {
  try {
    ...
    res.setHeader(
      'Content-Disposition',
      'attachment; filename=' + `CRM_Backup_${new Date().toISOString().split('T')[0]}.xlsx`
    );
...
exports.exportFullDatabaseJson = async (req, res) => {
  try {
    ...
    res.setHeader('Content-Disposition', `attachment; filename="CRM_Backup_${new Date().toISOString().split('T')[0]}.json"`);
