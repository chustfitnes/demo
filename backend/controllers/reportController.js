...
exports.getSalesReport = async (req, res) => {
  try {
    ...
    orders.forEach(order => {
       ...
       const pType = order.paymentType || 'naqd';
       if (paymentStats[pType] !== undefined) {
       ...
       order.items.forEach(item => {
         if (item.product && item.product._id) {
            ...
            if (!productStats[pId]) {
              ...
              if (pName === 'Brendsiz') pName = 'Maxsulot';
    ...
    for (let i = 0; i <= cappedDays; i++) {
      ...
      chartDataMap[dateStr] = { date: dateStr, displayDate, savdo: 0, foyda: 0 };
    }
    ...
    res.json({
       ...
       data: {
         ...
         chartData: hideSecret ? chartData.map(c => ({ ...c, foyda: 0 })) : chartData
       }
    });
...
exports.exportSalesExcel = async (req, res) => {
  try {
    ...
    const orders = await Order.find(match).populate('items.product', 'brand collection artikul unit').lean();
    ...
    workbook.creator = 'CRM Hisobotlar';
    ...
    kpi1Title.value = 'Jami Sotilgan Miqdor';
    ...
    // Foyda va Qarz (Agar yashirilmagan bo'lsa)
    const hideSecret = req.user && req.user.role === 'cashier';
    if (!hideSecret) {
      sheet.mergeCells('G4:H4');
      const kpi3Title = sheet.getCell('G4');
      kpi3Title.value = 'Sof Foyda (so\'m)';
      kpi3Title.font = { bold: true };
      kpi3Title.fill = { type: 'pattern', pattern: 'solid', fgColor: { argb: 'FFF3F4F6' } };
      
      sheet.mergeCells('G5:H5');
      const kpi3Value = sheet.getCell('G5');
      kpi3Value.value = totalProfit;
      kpi3Value.font = { size: 14, bold: true, color: { argb: 'FF7C3AED' } };
      kpi3Value.numFmt = '#,##0" so\'m"';
      
      sheet.mergeCells('J4:K4');
      const kpi4Title = sheet.getCell('J4');
      kpi4Title.value = 'Jami Qarz/Nasiya (so\'m)';
      kpi4Title.font = { bold: true };
      kpi4Title.fill = { type: 'pattern', pattern: 'solid', fgColor: { argb: 'FFF3F4F6' } };
      
      sheet.mergeCells('J5:K5');
      const kpi4Value = sheet.getCell('J5');
      kpi4Value.value = totalDebt;
      kpi4Value.font = { size: 14, bold: true, color: { argb: 'FFDC2626' } };
      kpi4Value.numFmt = '#,##0" so\'m"';
    }
